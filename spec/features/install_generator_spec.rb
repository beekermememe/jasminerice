...
feature "Installing JasmineCoffee", :aruba => true do

  before do
    unset_bundler_env_vars
    run_simple("bundle exec rails new testapp --skip-bundle")
    cd("testapp")
    append_to_file("Gemfile", %{\ngem "jasminecoffee", :path => "#{File.expand_path('../../../', __FILE__)}"\n})
    run_simple("bundle install --local")
  end

  scenario "installs the expected files" do
    run_simple("bundle exec rails generate jasminecoffee:install --trace")
    expected = <<-OUTPUT
      create  config/initializers/jasmine_coffee.rb
    ...
    check_file_presence(["config/initializers/jasmine_coffee.rb"], true)
